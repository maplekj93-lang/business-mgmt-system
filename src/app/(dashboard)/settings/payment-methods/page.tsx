...
import { Trash2, Plus, CreditCard, Eye, EyeOff } from 'lucide-react';
...
export default function PaymentMethodsSettingsPage() {
    ...
    const handleAddAsset = async () => {
        ...
        const newAsset = {
            name: '새 결제수단',
            asset_type: 'credit_card' as const,
        ...
        if (result.error) {
            toast.error('결제수단 추가 실패: ' + result.error);
        } else if (result.data) {
            setAssets(prev => [...prev, result.data as Asset]);
            toast.success('새 결제수단이 추가되었습니다');
        }
        setSaving(null);
    };

    const handleDeleteAsset = async (id: string) => {
        if (!confirm('이 결제수단을 삭제하시겠습니까? 거래 내역이 연결되어 있다면 문제가 발생할 수 있습니다.')) return;
        ...
        <div className="pb-20">
            <main className="container max-w-6xl mx-auto p-4 md:p-8 space-y-8 animate-in fade-in slide-in-from-bottom-4">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                    <div className="space-y-1">
                        ...
                        <p className="text-muted-foreground">
                            신용카드, 체크카드, 간편결제(페이) 등 결제에 사용되는 수단을 식별 키워드와 함께 관리하세요.
                        </p>
                    </div>
                        ...
                        <table className="w-full text-left text-sm border-collapse">
                            <thead>
                                <tr className="bg-white/5 border-b">
                                    ...
                                    <th className="px-4 py-3 font-semibold text-muted-foreground w-40">결제수단 이름</th>
                            ...
                            <tbody className="divide-y divide-white/5">
                                ...
                                {assets.length === 0 && (
                                    <tr>
                                        <td colSpan={7} className="p-8 text-center text-muted-foreground border-t">
                                            생성된 결제수단이 없습니다. '신규 생성'을 눌러 추가해주세요.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                ...
                <div className="bg-primary/5 border-primary/20 p-4 rounded-lg flex items-start gap-4 text-sm text-primary/80">
                    ...
                    <div className="space-y-1">
                        <p className="font-semibold">결제수단 및 키워드 연동 안내</p>
