...
export async function getDailyRateLogs(yearMonth?: string) {
    ...
    try {
        ...
        return { 
            success: true, 
            data: (data || []) as unknown as DailyRateLog[] 
        };
    } catch (error: unknown) {
        console.error('Failed to fetch daily rate logs:', error);
        return { success: false, error: '일당 기록을 불러오지 못했습니다.', data: [] as DailyRateLog[] };
    }
}
